...
// error_reporting(E_ALL ^ E_NOTICE);
	 ...
	 * by session => true
	 */
	define('CONFIG_ACCESS_CONTROL_MODE', false);  // using LiveCart sessions and checking if admin, see config.php
	...
	define('CONFIG_SYS_DEFAULT_PATH', '../../../../public/upload/files'); //accept relative path only
	define('CONFIG_SYS_ROOT_PATH', '../../../../public/upload/files');	//accept relative path only

	if (!file_exists(CONFIG_SYS_DEFAULT_PATH))
	{
		mkdir(CONFIG_SYS_DEFAULT_PATH, 0777);
		chmod(CONFIG_SYS_DEFAULT_PATH, 0777);
	}

	define('CONFIG_SYS_FOLDER_SHOWN_ON_TOP', true); //show your folders on the top of list if true or order by name

	define("CONFIG_SYS_DIR_SESSION_PATH", null); // using LiveCart session
	...
	define('CONFIG_EDITOR_NAME', 'tinymce');
